...
NSString * Ito_Nowa_Project3$ModuleRequireFormat = @"(function(exports){"
...
@implementation Ito_Nowa_Project3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ito_nowa_project3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Ito_Nowa_Project3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Ito_Nowa_Project3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ito_nowa_project3 = [[Ito_Nowa_Project3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ito_nowa_project3];
	
	NSString *_ito_nowa_project3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ito_nowa_project3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ito_nowa_project3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Ito_Nowa_Project3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.ito_nowa_project3.kroll" 
